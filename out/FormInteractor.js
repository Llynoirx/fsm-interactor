...
export class FormInteractor {
    ...
    createRotaryDial() {
        ...
        rotDial.addEventListener('mousedown', (e) => {
            isRotating = true;
            startAngle = Math.atan2(e.clientY - (rotDial.offsetTop + rotDial.offsetHeight), e.clientX - (rotDial.offsetLeft + rotDial.offsetWidth / 2));
        });
        //Check for mouse movement to rotate dial
        document.addEventListener('mousemove', (e) => {
            if (isRotating) {
                // Calculate and update curr angle, clamping it within -90 to 90 degrees
                const angle = Math.atan2(e.clientY - (rotDial.offsetTop + rotDial.offsetHeight), e.clientX - (rotDial.offsetLeft + rotDial.offsetWidth / 2));
                const deltaAngle = angle - startAngle;
                currAngle = Math.max(-Math.PI / 2, Math.min(Math.PI / 2, currAngle + deltaAngle));
    ...
    createSubmitButton(label, onClick) {
...
//# sourceMappingURL=FormInteractor.js.map